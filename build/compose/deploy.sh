...
args=`getopt -o h -a -l help,init,load,deploy,images:,jar: -n "${source_name}" -- "$@"`
...
do
    case "$1" in
        ...
        --images | -images)
            ...
            ;;
        --jar | -jar)
            info "option --jar argument : $2"
            arg_jar=$(getRealPath "$2")
            shift 2
            ;;
        --)
            shift
...
# define usage
usage=$"`basename $0` [-h|--help] [--init] [--load] [--deploy] [--images=xxx.tgz] [--jar=xxx.jar]
       [-h|--help]................show help info.
       [--init]...................init volume.
       [--load]...................load images.
       [--deploy].................deploy jar to datadir.
...
fun_execute_deploy_command() {
    ...
    if [ ! "x${suffix}" == "xjar" ]; then
        error "unsupported file format : ${arg_jar}"
        usage "$usage"
        return 1
    fi

    info "deploy data : ${arg_jar}"
    \rm -rf "${base_dir}/volume/single/data/"
    mkdir -p "${base_dir}/volume/single/data/"
    chmod -R 777 "${base_dir}/volume/single/data/"
    \cp -rfp "${arg_jar}" "${base_dir}/volume/single/data/"
    success "successfully deployed ${arg_jar}"
